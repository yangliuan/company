...
dim nameArray,h,e
dim valueArray(20)
function addRecord(sql,nameArray,valueArray,e)
...
function editShow(sql,nameArray,valueArray,e)
...
function updateRecord(sql,nameArray,valueArray,e)
   rs.open sql,conn,1,3
   for h=1 to e
   rs(nameArray(h))=valueArray(h)
   next
...
function writePage(fileName)'输出部分
  on error resume next
   response.Write("<table width='95%' height='24' align='center' cellpadding='0' cellspacing='0'><tr>")
if rs.pagecount > 0 then
   response.Write("<td width='15%' align='left'  height='24'>当前页"&intpage&"/"&rs.PageCount&"</td>")
else
...
function checkLogin(url)
 if session("checkLogin")<>true then
  response.Write("<script language='javascript'>alert('你还未登录，请先登录');window.location.href='"+url+"';</script>")     
  response.End()
 end if
end function
%>